...
def create_event(event_data, user_id):
    ...
    return event.to_json(max_nesting=1)


def get_event_by_id(event_id):
    event = backend.get_event_by_id(event_id)
    event_json = event.to_json(max_nesting=1)
    return event_json


def get_all_events(user_id):
    events = backend.get_all_events(user_id)
    list_of_events = [event.to_dict(max_nesting=1) for event in events]
...
def get_posts_from_event(event_id):
    ...
    list_of_posts = [event.to_dict(max_nesting=1) for event in posts]
