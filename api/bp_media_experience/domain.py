from . import backend
from ..helper_functions.get_media_by_id import get_experience_media_by_id
...
def get_media(user_id, media_experience_id):
...
def get_medias(user_id, experience_id):
    medias = backend.get_medias(user_id, experience_id)
    medias_list = [media.to_dict() for media in medias]
